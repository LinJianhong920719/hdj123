...
- (void)viewDidLoad
{
    ...
    btnBack = [[UIButton alloc]initWithFrame:CGRectMake(0, 0, 40, 25)];
    [btnBack setTitle:@"取消" forState:UIControlStateNormal];
    [btnBack setTitleColor:[UIColor blackColor] forState:UIControlStateNormal];
    btnBack.titleLabel.font = [UIFont systemFontOfSize:14];
    [btnBack setTitleEdgeInsets:UIEdgeInsetsMake(2.0, 10.0, 0.0, 0.0)];
    [btnBack addTarget:self action:@selector(doneAction:) forControlEvents:UIControlEventTouchUpInside];
...
- (IBAction)navBack:(id)sender {
    [[NSNotificationCenter defaultCenter] postNotificationName:@"navigationBarHidden" object:nil];
...
- (void)preparePhotos
{
    @autoreleasepool {
        ...
        dispatch_sync(dispatch_get_main_queue(), ^{
            ...
            //            [self.navigationItem setTitle:self.singleSelection ? NSLocalizedString(@"Pick Photo", nil) : NSLocalizedString(@"请选择图片", nil)];
            
            UILabel *titleView = [[UILabel alloc] initWithFrame:CGRectMake(60, 0, 180, 50)];
            titleView.text = @"请选择图片";
