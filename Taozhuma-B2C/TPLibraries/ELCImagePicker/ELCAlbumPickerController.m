...
//用户名
#define KEY_IMAGES_COUNT @"images_count"
...
- (BOOL)shouldSelectAsset:(ELCAsset *)asset previousCount:(NSUInteger)previousCount
{
    ...
    NSLog(@"%d",previousCount);
    
    NSInteger  DD=203-[Tools intForKey:KEY_IMAGES_COUNT];
    BOOL shouldSelect = previousCount < DD;
    if (!shouldSelect) {
        NSString *title = [NSString stringWithFormat:NSLocalizedString(@"最多能选择%d张图片", nil), 3];
        NSString *message = [NSString stringWithFormat:NSLocalizedString(@"您还能选择%d张图片", nil), DD];
