...
+ (HYBURLSessionTask *)_requestWithUrl:(NSString *)url
  ...
  if (httpMethod == 1) {
    ...
    session = [manager GET:url parameters:params progress:^(NSProgress * _Nonnull downloadProgress) {
      ...
      if (sg_cacheGet) {
        [self cacheResponseObject:responseObject request:task.currentRequest parameters:tempParams];
      }
...
+ (NetworkingRespond *)response:(NSDictionary *)response
                        success:(void (^)(id, NSString *))success
                           fail:(void (^)(NSString *))fail {
    NetworkingRespond *respond = [NetworkingRespond alloc];

    respond.success = [[response valueForKey:@"success"]intValue];
    respond.result = [response valueForKey:@"result"];
    respond.error_msg = [response valueForKey:@"error_msg"];
    
    if (respond.success) {
        success(respond.result, respond.error_msg);
        fail(respond.error_msg);
    } else {
        success(nil, respond.error_msg);
        fail(respond.error_msg);
    }
